...
export async function getLibrarie(_mpn, _package, { extra, getState }) {
    let api = extra.api;
    ...
    try {
        ...
        if (buffer && buffer.byteLength > 100) {
            ...
            for (let entry of entries) {
                if (entry.filename && entry.filename.toLocaleLowerCase().includes('kicad') && entry.filename.endsWith(".lib")) {
...
export default createAction(getLibrarie);